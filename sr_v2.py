...
def update_support_resistance_levels(symbol: str, timeframe: str):
    ...
    support_with_strength = [(level, calculate_level_strength(level, candles, CONFIG["zone_width_percent"])) for level in clustered_support]
    resistance_with_strength = [(level, calculate_level_strength(level, candles, CONFIG["zone_width_percent"])) for level in clustered_resistance]
    ...
    def filter_close_levels(levels, min_distance_percent):
        ...
        for level, strength in levels:
            if not filtered or all(abs(level - existing) / existing > min_distance_percent for existing, _ in filtered):
...
async def send_fcm_alert(sound_type: str, message: str):
    if not CONFIG["alerts"]["fcm"]:
        return

    try:
        tokens = []
        if os.path.exists(CONFIG["DEVICE_TOKENS_FILE"]):
            with open(CONFIG["DEVICE_TOKENS_FILE"], "r") as f:
                tokens = json.load(f).get("tokens", [])
                
        if not tokens:
            print("[FCM] No device tokens available")
            return
            
        for token in tokens:
            msg = messaging.Message(
                notification=messaging.Notification(
                    title="Support/Resistance Alert",
                    body=message
                ),
                data={"trigger_key": sound_type},
                token=token
            )
...
async def trigger_support_alert(symbol: str, timeframe: str, level: float, strength: float, current_price: float):
    ...
    await send_fcm_alert(sound_type, message)
